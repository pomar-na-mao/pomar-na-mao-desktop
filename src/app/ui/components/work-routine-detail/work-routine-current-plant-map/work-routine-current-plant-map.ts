...
@Component({
  ...
  templateUrl: './work-routine-current-plant-map.html',
  host: {
    'style': 'display: contents'
  }
})
export class WorkRoutineCurrentPlantMap implements OnInit, AfterViewInit, OnDestroy {
  @Input() id!: number;

  private platformId = inject(PLATFORM_ID);
  private repository = inject(WorkRoutinePlantsRepository);
  ...
  constructor() {
    effect(() => {
      ...
      const selectedPlant = this.repository.selectedWorkRoutinePlant();
  ...
  public async ngOnInit(): Promise<void> {
    await Promise.all([
      this.workRoutineRepository.fetchWorkRoutines(),

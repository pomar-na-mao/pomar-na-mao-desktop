...
export class RoutineSyncViewModel {
  ...
  public currentRoutinePlant =
    this.routinePlantsRepository.selectedRoutinePlant;
  public totalPlants = computed(
    () => this.routinePlantsRepository.routinePlants().length,
  );
  ...
  public occurrencesChanges = computed(() => {
    ...
    const equivalentPlant = plants.find(
      (plant) => plant.id === currentRoutinePlant.plant_id,
    );

    for (const key of occurenceKeys) {
      const routineValue =
        currentRoutinePlant[key as keyof typeof currentRoutinePlant];
      const plantValue = equivalentPlant
        ? equivalentPlant[key as keyof typeof equivalentPlant]
        : false;
      ...
      try {
        ...
        const occurrences = Object.fromEntries(
          occurenceKeys.map((key) => [
            key,
            plantWithUpdates[key as keyof typeof plantWithUpdates],
          ]),
        ) as {
          [k: string]: boolean;
        };
        ...
        const { error } =
          await this.routinePlantsRepository.updatePlantFromRoutine(
            plantId,
            occurrences,
            routinePlantId,
            informations,

...
create index if not exists idx_regions_zone_id_order
  on public.regions using btree (zone_id, "order");
...
begin
  ...
  select
    ...
    v_zone_id,
    coalesce(
      nullif(point->>'order', '')::integer,
      point_ordinality::integer
    )
  from jsonb_array_elements(p_points) with ordinality as points(point, point_ordinality);

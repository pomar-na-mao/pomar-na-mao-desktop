...
begin
  if not exists (
    ...
    where table_schema = 'public'
      and table_name = 'plant_occurrences'
      and column_name in ('status', 'resolved_at', 'updated_at')
    group by table_schema, table_name
    having count(*) = 3
  ) then
    raise exception 'plant_occurrences must expose status, resolved_at and updated_at for occurrenceAction remove';
  end if;

  if not has_function_privilege('authenticated', 'public.sync_polygon_bulk_update(jsonb)', 'execute') then
    raise exception 'authenticated must execute public.sync_polygon_bulk_update(jsonb)';
  end if;

  if has_function_privilege('anon', 'public.sync_polygon_bulk_update(jsonb)', 'execute') then
    raise exception 'anon must not execute public.sync_polygon_bulk_update(jsonb)';
  end if;
end;
$$;
